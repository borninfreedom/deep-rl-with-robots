...
class KukaReachEnv(gym.Env):
    ...
    def __init__(self, config:EnvContext):
        ...
        self.observation_space = spaces.Box(
            low=np.array([self.x_low_obs_for_obs_space, self.y_low_obs_for_obs_space, self.z_low_obs_for_obs_space]),
            high=np.array([self.x_high_obs_for_obs_space, self.y_high_obs_for_obs_space, self.z_high_obs_for_obs_space]),
            dtype=np.float32)
        self.step_counter = 0
    ...
    def _reward(self):
        ...
        return self._get_observation(), reward, self.terminated, {}
